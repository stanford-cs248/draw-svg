                                             ...
                                             Color color)
{
  ...
  float b1 = -(x2 - x1);
  float b2 = -(x0 - x2);
  float c0 = -x0 * a0 - y0 * b0;
  float c1 = -x1 * a1 - y1 * b1;
  float c2 = -x2 * a2 - y2 * b2;

  int x_min = (int)floor(min(min(x0, x1), x2));
  int y_min = (int)floor(min(min(y0, y1), y2));
  int x_max = (int)floor(max(max(x0, x1), x2));
  int y_max = (int)floor(max(max(y0, y1), y2));

  for (int sx = x_min; sx <= x_max; sx++)
  {
    for (int sy = y_min; sy <= y_max; sy++)
    {
      // determine whether a given sample point (sx,sy) is inside the triangle
      if ((a0 * sx + b0 * sy + c0 >= 0 && a1 * sx + b1 * sy + c1 >= 0 && a2 * sx + b2 * sy + c2 >= 0) ||
          (a0 * sx + b0 * sy + c0 <= 0 && a1 * sx + b1 * sy + c1 <= 0 && a2 * sx + b2 * sy + c2 <= 0))
      {
